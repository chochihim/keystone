...
class MongooseAdapter extends BaseKeystoneAdapter {
  ...
  async _createAdjacencyTable({ left, tableName }) {
    ...
    // console.log(left.adapter.refListKey);
    // console.log(right.adapter.refListKey);
    // console.log({ leftFkPath, rightFkPath });
    // console.log(leftListAdapter.key);
    // console.log(rightListAdapter.key);
...
class MongooseListAdapter extends BaseListAdapter {
    ...
    if (cardinality === '1:1') {
      ...
      if (values.length) {
        if (cardinality === 'N:N') {
          ...
          return (await this._getModel(tableName).create(
            values.map(id => ({
          ...
          const res = await this._getModel(tableName).updateMany(
            { _id: { $in: values } },
            { [columnName]: itemId }
          );
          return values;
  ...
  async _update(id, data) {
    ...
    await this._processNonRealFields(data, async ({ value: newValues, adapter }) => {
      ...
      if (cardinality !== '1:1') {
        ...
        console.log('NEED TO DELETE SOME THINGS?', item);
        console.log(await this._getModel(tableName).find());
        const currentRefIds = await this._getModel(tableName).aggregate([
        ...
        console.log({ needsDelete });
        if (needsDelete.length) {
          if (cardinality === 'N:N') {
            console.log('FIXME: N:N');
          } else {
            console.log('FIXME: 1:N');
          }
        }
      ...
      Object.values(this.parentAdapter.listAdapters).map(adapter =>
        Promise.all(
          adapter.fieldAdapters
            .filter(a => a.isRelationship && a.refListKey === this.key) // If I (a list adapter) an implicated in the .rel of this field adapter
            .map(a => {
  ...
  async _itemsQuery(args, { meta = false, from, include } = {}) {
    ...
    if (from && Object.keys(from).length) {
      // console.log(from.fromList.adapter.fieldAdaptersByPath);
      ...
      if (cardinality === 'N:N') {
        ...
        // console.log({ $where: {[`${from.fromList.adapter.key}_id`]: from.fromId } });
        // console.log(this._getModel(tableName) );
        // try{
        ...
        // console.log({ ids });
        // console.log(ids.map(x => x[`${this.key}_id`]));
      ...
      // ids = await from.fromList.adapter._itemsQuery(
    ...
    this.fieldAdapters
      ...
      .forEach(a => {
        ...
        // console.log(this._getModel(tableName).collection.name);
